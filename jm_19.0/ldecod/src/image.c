  ...
  while(current_header != SOP && current_header !=EOS)
  {
	...
	if (xml_gen_trace_file())
	{
		...
		if (is_new_picture(p_Vid->dec_picture, currSlice, p_Vid->old_slice))
		{
            ...
            case FRAME: structure_type = 'F'; break;//xml_write_int_attribute("structure", 0); 
			case TOP_FIELD: structure_type = 'T';break;//xml_write_int_attribute("structure", 1); 
			case BOTTOM_FIELD: structure_type = 'B'; break; //xml_write_int_attribute("structure", 2); 
			}
...
void decode_one_slice(Slice *currSlice)
{
  ...
  if (xml_gen_trace_file())
  {
	  ...
	  switch (p_Vid->type)
	  {
	  case B_SLICE: slice_type='B'; break;
	  case I_SLICE: slice_type = 'I'; break;
	  case P_SLICE: slice_type = 'P'; break;
	  case SI_SLICE: slice_type = 'L'; break;
	  case SP_SLICE: slice_type = 'Q'; break;
	  }
